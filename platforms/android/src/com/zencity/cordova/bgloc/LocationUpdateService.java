...
public class LocationUpdateService extends Service implements LocationListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            persistLocation = Boolean.parseBoolean(intent.getStringExtra("persistLocation"));
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                notification = buildForegroundNotificationWithChannel(builder);
        ...
        Log.i(TAG, "- persistLocation: "   + persistLocation);
        this.setPace(false);
    ...
    @TargetApi(26)
    private Notification buildForegroundNotificationWithChannel(Notification.Builder builder) {

        NotificationManager notificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        NotificationChannel notificationChannel = new NotificationChannel(CHANNEL, notificationTitle, NotificationManager.IMPORTANCE_DEFAULT);
    ...
    public void deleteAllLocations(Intent intent){
        Log.d(TAG, "Deleting all Locations");
          if (isDebugging) {
                    Toast.makeText(this, "Deleting all Locations", Toast.LENGTH_SHORT).show();
                }
        deleteLocations();
    ...
    public void onLocationChanged(Location location) {
        ...
        if(persistLocation) {
            ...
            if (this.isNetworkConnected()) {
                Toast.makeText(this, "Scheluding POST:  " + persistLocation, Toast.LENGTH_LONG).show();
            ...
            if(isDebugging)
                Toast.makeText(this, "Posting Current Location", Toast.LENGTH_LONG).show();
            //Sending data to server, directly
            PostCurrentLocationTask postCurrentLocationTask = new LocationUpdateService.PostCurrentLocationTask();
            postCurrentLocationTask.execute(location);
        }
    }
    ...
    */
    ...
    private boolean postCurrentLocation(Location l) {
        if (l == null) {
    ...
    private class PostCurrentLocationTask extends AsyncTask<Location, Integer, Boolean> {

        @Override
        protected Boolean doInBackground(Location...locations) {
            Log.d(TAG, "Executing PostCurrentLocationTask#doInBackground");
            for (Location l: locations)
            {
                postCurrentLocation(l);
            }
